...
def visualize(tensor, title=""):
    tensor_ = tensor.detach().cpu().flatten().numpy()
    plt.hist(tensor_, bins=100)
    plt.title(title)
    plt.show()

# hidden_states[1].reshape(hidden_states.shape[1], -1).std(-1).topk(10)
# hidden_states[1][980, :, :]
def visualize_img(tensor, title=""):
    _max = tensor.max()
    _min = tensor.min()
    tensor = (tensor - _min) / (_max - _min)

    tensor_ = tensor.detach().cpu().numpy().astype(float)
    plt.imshow(tensor_)
    plt.colorbar()
    plt.title(title)
    plt.show()
...
class ResnetBlock2D(nn.Module):
    ...
    def forward(self, input_tensor, temb):
        ...
        # input_tensor and hidden_states has different std
...
class Downsample2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # I tried. and not sensitive to std. (small values are negligible)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python sd.py flops [unet|decoder]")
        exit()
        ...
        if sys.argv[2] =='unet':
            ...
            example_inputs = {
                ...
                'added_cond_kwargs': {
                    ...
                    'time_ids': torch.randint(0, 1000, (1, 6)),
                }
            }
            ...
            pipe = StableDiffusionXLPipeline.from_pretrained("stabilityai/sdxl-turbo").to('cuda')
